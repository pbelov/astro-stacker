...
rem
rem В папку кладутся обе программы: командная строка и окно. Декодеры вкомпилены
rem в обе, класть рядом нечего. Проверка в конце запускает собранный бинарник и
rem спрашивает, какие форматы он читает: собранный, но ничего не читающий
rem бинарник запускается и печатает справку как ни в чём не бывало.
rem
...
rem Настоящая проверка, а не формальность: бинарник, собравшийся без декодера,
rem запускается, печатает справку и выглядит здоровым - и молча не читает ни
rem одного кадра. Такую сборку можно отдать и узнать о поломке от того, кто её
rem открыл.
rem
rem Спрашивается командная строка, но отвечает она за обе программы: декодеры
rem собираются из одних и тех же крейтов. Чего проверка не покрывает - окно
rem само: оно оконное, из скрипта его не спросить, и ему вдобавок нужен
