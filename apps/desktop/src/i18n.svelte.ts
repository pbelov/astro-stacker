...
const ru = {
  ...
  outNotSet: "ещё не сохранён",

  aligning: "Совмещаю кадры…",
  stackingFrame: "Складываю {done} из {total} — {name}",
  finishing: "Готовлю результат…",
  stackSlow: "Четыре прохода по кадрам, два из них с декодированием. Прогон в пару сотен кадров — это минуты.",
  resultWaiting: "Готово. Результат — на соседнем шаге, там же и сохранение.",

  stepResult: "Результат",
  noResultYet: "Пока нечего показать: результат появится здесь, когда пройдёт сложение.",
  keepIt: "Что оставить",
  keepHint: "Сложенный кадр держится в памяти, так что сохранить можно любой вид и в любом порядке, хоть все три. Каждый — свой проход по всем пикселям, поэтому платишь только за то, что взял.",
  saving: "Записываю…",
  whatWasStacked: "Что сложилось",
  stackedSummary: "{light} в {frames} {frames:кадре|кадрах|кадрах}",
  statLight: "Света набрано",
  lightBreakdown: "отснято {shot}, по глубине с учётом весов — {effective}",
  exposureMissing: "{n} {n:кадр не записал|кадра не записали|кадров не записали} свою выдержку, так что итог — не меньше показанного",
  statFrames: "Кадров",
  effectiveDepth: "по глубине это {n} {n:кадр|кадра|кадров} медианного качества",
  statCanvas: "Холст",
  statTook: "Заняло",
  coverage: "Покрытие по плоскостям",
  coverageHint: "Кадры кладутся после совмещения, так что к краям их меньше, чем в середине: одна цифра на весь кадр была бы неправдой.",
...
const en: Record<Keys, string> = {
  ...
  resultWaiting: "Done. The result is on the next step, and so is saving it.",

  stepResult: "Result",
  noResultYet: "Nothing to show yet: the result appears here once a stack has run.",
  keepIt: "What to keep",
  keepHint: "The stack is held in memory, so any of these can be saved, in any order, or all three. Each is its own pass over every pixel, so you pay only for what you take.",
  saving: "Writing…",
  whatWasStacked: "What was stacked",
  stackedSummary: "{light} in {frames} {frames:frame|frames}",
  statLight: "Light collected",
  lightBreakdown: "{shot} shot, {effective} once weights are taken into account",
  exposureMissing: "{n} {n:frame|frames} recorded no exposure, so the totals are a floor rather than the whole",
  statFrames: "Frames",
  effectiveDepth: "worth {n} {n:frame|frames} of median quality",
  statCanvas: "Canvas",
  statTook: "Took",
  coverage: "Coverage per plane",
  coverageHint: "Frames are laid down after alignment, so the edges are covered by fewer of them than the middle: one figure for the whole frame would be a lie.",
